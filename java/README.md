...
Este directorio contiene todo el material de **Java** de tus clases particulares: teoría por unidades (UT), ejercicios por nivel, proyectos por centro y documentación de apoyo.

### Agradecimientos especiales

Inspirado en el temario de **DAW 2023/2025** y en el trabajo de profesores como **Jaime García Martín Cuerva** y **Mercedes Urios de las Heras**, cuyo enfoque en buenas prácticas, patrones y arquitectura ha marcado la forma en la que está organizado este proyecto.
...
- **EV1 (src/ev1)**: introducción al lenguaje, sintaxis básica, tipos de datos, control de flujo, funciones, strings y una primera toma de contacto con la POO.
- **EV2 (src/ev2)**: estructuras de datos básicas (arrays, ArrayList, clase `Array`) y ejercicios clásicos que consolidan el uso de colecciones y funciones.
- **EV3 (src/ev3)**: POO media y avanzada (abstracción, herencia, polimorfismo, colecciones), estructuras de datos formales, multihilo, JDBC/acceso a datos y proyectos por centro.
- **Pruebas libres (src/pruebaslibres)**: material específico para exámenes de recuperación completos.

La guía detallada de la **EV1** (explicaciones largas de cada apartado) está en `src/ev1/docs/GUIA-EV1.md`.
...
1. **Navegar por evaluación**: Entra en `src/ev1`, `src/ev2` o `src/ev3` según el trimestre o nivel que quieras trabajar.
2. **Navegar por unidad teórica (UT)**: Dentro de cada evaluación, usa `ut0`, `ut1`, `ut2`, etc. para localizar la teoría y los ejercicios asociados a cada bloque de contenidos.
3. **Revisar documentación de apoyo**: En cada evaluación, el directorio `docs/` contiene PDFs y materiales extra (funciones, strings, arrays, POO, estructuras de datos, JDBC, multihilo, etc.).
4. **Usar material por centro**: En `src/ev3/por_centro/` encontrarás proyectos y ejercicios completos organizados por comunidad y centro (por ejemplo, UAX, IES Velázquez, IES Calderón Barca, etc.).
5. **Preparar pruebas libres**: En `src/pruebaslibres/` tienes teoría y práctica pensadas para exámenes de recuperación de curso completo.
6. **Compilar y ejecutar**: La mayoría de ejercicios son independientes y pueden compilarse directamente con `javac` y ejecutarse con `java`, o abrirse en tu IDE favorito.
...
## Guía detallada de la EV1 (UT0 completa)

> La siguiente sección es la guía teórica extendida de la **Primera evaluación (EV1)**, unidad **UT0**: conceptos básicos de Java, ciclo de vida de un programa, sintaxis mínima, comentarios, tipos de datos y casteo. El código asociado está en `src/ev1/ut0/*`.

<!-- Contenido original de GUIA-EV1.md -->
...
* [**Segunda evaluación (EV2)**](#segunda-evaluación-ev2)
* [**Tercera evaluación (EV3)**](#tercera-evaluación-ev3)
...
En el código del proyecto encontrarás ejemplos prácticos de estas conversiones en la carpeta `src/ev1/ut0/casteotipos`, por ejemplo en el archivo `CasteoTiposPrimitivos.java`.
