package general.condicionales;

/**
 * Teoría: cuándo usar {@code if}, {@code if-else} o {@code switch}.
 *
 * <p><b>Objetivo:</b> elegir la estructura condicional adecuada.</p>
 *
 * <ul>
 *   <li><b>if</b> — una sola condición o comprobaciones independientes.</li>
 *   <li><b>if-else</b> — exactamente dos alternativas (o cadenas if-else if).</li>
 *   <li><b>switch</b> — muchos casos sobre el mismo valor (menú, día, opción numérica).</li>
 * </ul>
 *
 * <p>Demos: {@link IfIntro}, {@link IfElseIntro}, {@link SwitchIntro}.
 * Ejercicios: {@code ejercicios/if/}, {@code ejercicios/ifelse/}, {@code ejercicios/switch/}.</p>
 *
 * @author Agustín. A. Marquez. Piña
 * @since 29/05/2026
 * @see <a href="mailto:agu1406@outlook.es">agu1406@outlook.es</a>
 * @see <a href="https://github.com/Agu1406/ClasesParticulares">Repositorio GitHub</a>
 * @see <a href="https://www.agustinmarquez.dev">Sitio web</a>
 */
public class CondicionalesComparacionIntro {

    private CondicionalesComparacionIntro() {
    }
}
