 ...
 */
public class GestionAlumnosClase_SIN_RESOLVER {
    
    private static ArrayList<AlumnoClase> listaAlumnos = new ArrayList<>();
    ...
    private static void pedirDatosAlumnos(int numeroAlumnos) {
        ...
        for (int indice = 0; indice < numeroAlumnos; indice++) {
            ...
            AlumnoClase alumno = new AlumnoClase(nombre, nota);
    ...
    private static void mostrarMejorAlumno() {
        ...
        AlumnoClase mejorAlumno = listaAlumnos.get(0);
    ...
    private static void mostrarAlumnosOrdenados() {
        ...
        ArrayList<AlumnoClase> alumnosOrdenados = new ArrayList<>(listaAlumnos);
        
        // Ordenar por nota de mayor a menor usando Comparator
        alumnosOrdenados.sort(new Comparator<AlumnoClase>() {
            @Override
            public int compare(AlumnoClase a1, AlumnoClase a2) {
        ...
        for (int posicion = 0; posicion < alumnosOrdenados.size(); posicion++) {
            AlumnoClase alumno = alumnosOrdenados.get(posicion);
...
class AlumnoClase {
    ...
    /**
     * Constructor de la clase AlumnoClase
     */
    public AlumnoClase(String nombre, double nota) {
        this.nombre = nombre;
        this.nota = nota;
    }
