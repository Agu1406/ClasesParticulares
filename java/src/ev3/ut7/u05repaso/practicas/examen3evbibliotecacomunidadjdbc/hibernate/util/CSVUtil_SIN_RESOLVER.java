package ev3.ut7.u05repaso.practicas.examen3evbibliotecacomunidadjdbc.hibernate.util;

import ev3.ut7.u05repaso.practicas.examen3evbibliotecacomunidadjdbc.hibernate.model.Libro;

import java.io.IOException;
import java.util.List;

/**
 * Plantilla alumno: completar la exportacion CSV del examen.
 *
 * <p>En tu proyecto renombra esta clase a {@code CSVUtil} o copia el metodo resuelto.</p>
 */
public final class CSVUtil_SIN_RESOLVER {

    private CSVUtil_SIN_RESOLVER() {
    }

    /**
     * @param listaLibros libros a volcar
     * @param nombreFichero ruta del CSV de salida
     */
    public static void exportarLibros(List<Libro> listaLibros, String nombreFichero) throws IOException {
        // TODO: PrintWriter, cabecera isbn;paginas;tipo;anio y una fila por libro
    }
}
