...
import MyButton from '../08-actualizar-pagina/MyButton';

/**
 * AÑADIR ESTILOS (hoja compartida: `src/App.css` importada desde `App.tsx`).
 */
import AdminPanel from '../08-actualizar-pagina/AdminPanel';
import LoginForm from '../08-actualizar-pagina/LoginForm';
 ...
 * 
 * En lugar de que los componentes MyButton tengan cada uno su propio "useState"
 * renderizado y por lo tanto sin compartir datos, vamos a importar "useState"
 * aquí y crear nuestra propia función handleClick para que los botones usen
 * el mismo sistema de datos y compartan los mismos.
 */
import { useState } from 'react';

/**
 * Para no mezclar botones creamos otro botón sin su pripio "useState" y que
 * recibe como propiedades desde el padre (MyApp.tsx) las atributos count y
 * onClick que son, el contador y la función incrementadora.
 */
    ...
    return (
        <main>
            ...
            {/**
              * UTILIZACION DE HOOKS.
              * 
              * Los "hooks" son las funciones nativas de react que empiezan con la palabra "use"
              * ya usamos una de ellas de hecho, "useState" es lo que usamos para modificar el
              * estado del componente "MyButton" al incrementar el valor del contador cuando
              * hacemos click.
              * 
              * Cada uno de ellos tiene su propio "estado", si hago click 10 veces en un botón y
              * 5 en el otro, cada uno mostrara en su contador su propia cantidad de click, es
              * decir que no comparten información.
              * 
              * Si queremos que compartan información en lugar de renderizar "X" veces el mismo
              * componente con su propio "useState" tenemos que usar "useState" en el contenedor
              * padre.
              * 
              * Por eso declaramos aquí mi app nuestra propia funcion "handleClick" e importamos
              * "useState" de React.
              */}
            <h2 className="learn-title">Utilización de hooks en React</h2>
            <p className="learn-info">
                A veces queremos que nuestros componentes compartan datos, "useState" es uno de
                los varios hooks nativos de react que existen, cuando renderizamos como en las
                lecciones anteriores los componentes cada uno con su propio "useSate" cada uno
                lleva un registro de datos / acciones individuales, por eso, si hacemos click
                en dos botones del mismo componente MyButton cada uno llevara un registro de su
                propio contador sin solaparse con los otros, ejemplo:
            </p>
            <div className="learn-react">
                <MyButton />
                <MyButton />
            ...
            <p className="learn-info">
                Sin embargo, si usamos "useState" desde el contenedor padre de los componentes,
                en este caso "MyApp.tsx" y la función "handleClick" en el padre entonces si que
                comparten datos, para ello creamos un nuevo componente (MyButton2) que recibe
                como argumento propiedades del padre (MyApp.tsx) que son la variable contador
                y la función que incrementa dicho contador, quedando así:
            </p>
            <div className="learn-react">
